...
export default function EventDetail() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-['Open_Sans'] antialiased flex flex-col items-center uppercase selection:bg-purple-500 selection:text-white relative overflow-hidden">
      
      {/* LUCES DINÁMICAS DE FONDO ESTILO HOME */}
      ...
      <div className="w-full max-w-[1200px] px-4 sm:px-8 mt-6 relative z-10">
        <div className="relative w-full min-h-[240px] sm:min-h-[280px] bg-[#0e0e10] border border-white/15 rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden group shadow-[0_0_40px_rgba(0,0,0,0.4)] flex flex-col justify-end">
          ...
          <div className="relative z-10 p-6 sm:p-10 w-full flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
            ...
            <button onClick={() => setShowPhotoModal(true)} className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-[8px] font-black tracking-[0.2em] flex items-center justify-center gap-2 backdrop-blur-md transition-all cursor-pointer text-white shadow-lg">
      ...
      <div className="w-full max-w-[1200px] px-4 sm:px-8 mt-6 relative z-10">
        <div className="grid grid-cols-3 gap-2 sm:flex sm:gap-3 font-['Poppins']">
          <button onClick={() => setActiveTab('tables')} className={`py-3.5 px-2 sm:px-6 rounded-xl sm:rounded-2xl text-[8px] sm:text-[9px] font-black tracking-widest transition-all flex items-center justify-center gap-1.5 sm:gap-2 shadow-lg cursor-pointer ${activeTab === 'tables' ? 'bg-purple-600 text-white shadow-[0_0_20px_rgba(168,85,247,0.4)]' : 'bg-white/5 text-gray-400 hover:text-white border border-white/10'}`}>
            <LayoutGrid size={13}/> <span className="truncate">PLANO</span>
          </button>
          <button onClick={() => setActiveTab('guests')} className={`py-3.5 px-2 sm:px-6 rounded-xl sm:rounded-2xl text-[8px] sm:text-[9px] font-black tracking-widest transition-all flex items-center justify-center gap-1.5 sm:gap-2 shadow-lg cursor-pointer ${activeTab === 'guests' ? 'bg-purple-600 text-white shadow-[0_0_20px_rgba(168,85,247,0.4)]' : 'bg-white/5 text-gray-400 hover:text-white border border-white/10'}`}>
            <Users size={13}/> <span className="truncate">INVITADOS</span>
          </button>
          <button onClick={() => setActiveTab('budget')} className={`py-3.5 px-2 sm:px-6 rounded-xl sm:rounded-2xl text-[8px] sm:text-[9px] font-black tracking-widest transition-all flex items-center justify-center gap-1.5 sm:gap-2 shadow-lg cursor-pointer ${activeTab === 'budget' ? 'bg-purple-600 text-white shadow-[0_0_20px_rgba(168,85,247,0.4)]' : 'bg-white/5 text-gray-400 hover:text-white border border-white/10'}`}>
        ...
        {activeTab === 'tables' && (
          <div className="space-y-6">
            <form onSubmit={handleAddTable} className="bg-[#0e0e10] border border-white/15 p-4 sm:p-6 rounded-[1.75rem] sm:rounded-[2rem] grid grid-cols-1 sm:grid-cols-3 gap-3 font-bold items-center shadow-[0_0_30px_rgba(0,0,0,0.3)]">
              <input placeholder="NOMBRE (EJ: MESA 1)" value={newTable.name} onChange={e => setNewTable({...newTable, name: e.target.value})} className="bg-white/5 border border-white/15 rounded-xl p-3.5 text-[9px] text-white outline-none focus:border-purple-500 tracking-widest uppercase placeholder:text-gray-600" required />
              <input type="number" placeholder="CAPACIDAD (EJ: 10)" value={newTable.capacity} onChange={e => setNewTable({...newTable, capacity: e.target.value})} className="bg-white/5 border border-white/15 rounded-xl p-3.5 text-[9px] text-white outline-none focus:border-purple-500 tracking-widest uppercase placeholder:text-gray-600" required />
              <button type="submit" className="py-3.5 bg-purple-600 text-white rounded-xl text-[8px] font-black tracking-widest hover:bg-purple-500 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg">
              ...
              <div className="flex flex-col gap-4 border-b border-white/10 pb-5 font-['Poppins']">
                <div className="flex justify-between items-start gap-4">
                  <div className="space-y-0.5">
                    <span className="text-[7px] tracking-[0.3em] font-bold text-purple-400 uppercase">DISTRIBUCIÓN</span>
                ...
                <div className="grid grid-cols-3 gap-1 bg-white/5 border border-white/10 p-1 rounded-xl">
                  ...
                  <button onClick={() => setLayoutMode('auditorium')} className={`py-2 text-[7px] font-black tracking-widest rounded-lg transition-all cursor-pointer ${layoutMode === 'auditorium' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-400 hover:text-white'}`}>
                    TEATRO
                  </button>
                  <button onClick={() => setLayoutMode('standing')} className={`py-2 text-[7px] font-black tracking-widest rounded-lg transition-all cursor-pointer ${layoutMode === 'standing' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-400 hover:text-white'}`}>
                    CAMPO
                  </button>
                </div>
              </div>
                          ...
                          <div key={t.id} className="bg-white/5 border border-white/10 rounded-[2rem] p-5 space-y-3 shadow-xl flex flex-col items-center relative group backdrop-blur-md">
                            ...
                            <div className="w-full flex justify-between items-center border-b border-white/10 pb-2.5">
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-[7px] text-purple-300 font-bold bg-white/5 px-2 py-0.5 rounded-full border border-white/10">{assignedGuests.length}/{capacity}</span>
                                    ...
                                    className={`absolute w-5 h-5 rounded-full flex items-center justify-center text-[6px] font-black border transition-all shadow-md ${
                                      assignedGuest
                                        ? assignedGuest.status === 'CONFIRMADO'
                                          ? 'bg-purple-600 text-white border-purple-400 scale-110 z-20 shadow-[0_0_10px_rgba(168,85,247,0.5)]'
                              ...
                              {assignedGuests.length === 0 ? (
                                ...
                                assignedGuests.map(g => (
                                  <div key={g.id} className="flex justify-between items-center text-[7px] text-gray-300 font-bold px-1">
                                    ...
                                    <span className={`text-[6px] px-1 rounded font-black ${g.status === 'CONFIRMADO' ? 'bg-purple-600/30 text-purple-300 border border-purple-500/30' : 'bg-amber-500/20 text-amber-300'}`}>{g.status}</span>
                                  </div>
                                ))
                              )}
                            </div>
                          ...
                          return (
                            <div key={t.id} className="bg-white/5 border border-white/10 rounded-2xl p-4 space-y-3 backdrop-blur-md">
                              <div className="flex justify-between items-center border-b border-white/10 pb-2.5">
                                ...
                                <div className="flex items-center gap-2">
                                  <span className="text-[7px] text-purple-300 font-bold">{assignedGuests.length}/{capacity}</span>
                            ...
                            <div key={t.id} className="bg-white/5 border border-white/10 rounded-2xl p-4 space-y-3 backdrop-blur-md">
                              ...
                              <div className="space-y-1">
                                ...
                                <div className="w-full h-1.5 bg-[#0a0a0a] rounded-full overflow-hidden border border-white/10">
                                  <div className="h-full bg-gradient-to-r from-purple-500 to-indigo-500 transition-all duration-500 shadow-[0_0_10px_rgba(168,85,247,0.5)]" style={{ width: `${percentage}%` }}></div>
                                </div>
                              </div>
                                ...
                                {assignedGuests.length === 0 ? (
                                  ...
                                  assignedGuests.map(ag => (
                                    <div key={ag.id} className="text-[7px] text-gray-200 bg-white/5 px-2 py-1 rounded-lg border border-white/10 flex justify-between items-center font-bold">
                                      ...
                                      <span className={`text-[6px] px-1 rounded font-black ${ag.status === 'CONFIRMADO' ? 'bg-purple-600/30 text-purple-300 border border-purple-500/30' : 'bg-amber-500/20 text-amber-300'}`}>{ag.status}</span>
                                    </div>
                                  ))
                                )}
                              </div>
        ...
        {activeTab === 'guests' && (
          <div className="space-y-5">
            <form onSubmit={handleAddGuest} className="bg-[#0e0e10] border border-white/15 p-4 sm:p-6 rounded-[1.75rem] sm:rounded-[2rem] grid grid-cols-1 sm:grid-cols-4 gap-3 font-bold items-center shadow-[0_0_30px_rgba(0,0,0,0.3)]">
              <input placeholder="NOMBRE Y APELLIDO" value={newGuest.name} onChange={e => setNewGuest({...newGuest, name: e.target.value})} className="bg-white/5 border border-white/15 rounded-xl p-3.5 text-[9px] text-white outline-none focus:border-purple-500 tracking-widest uppercase placeholder:text-gray-600" required />
              <select value={newGuest.table} onChange={e => setNewGuest({...newGuest, table: e.target.value})} className="bg-[#0e0e10] border border-white/15 rounded-xl p-3.5 text-[9px] text-white outline-none focus:border-purple-500 tracking-widest uppercase cursor-pointer">
                {tables.length === 0 ? <option value="">SIN SECTORES</option> : tables.map(t => <option key={t.id} value={t.name}>{t.name}</option>)}
              </select>
              <select value={newGuest.status} onChange={e => setNewGuest({...newGuest, status: e.target.value})} className="bg-[#0e0e10] border border-white/15 rounded-xl p-3.5 text-[9px] text-white outline-none focus:border-purple-500 tracking-widest uppercase cursor-pointer">
              ...
              {guests.map(g => (
                <div key={g.id} className="bg-[#0e0e10] border border-white/15 p-5 rounded-2xl flex justify-between items-center shadow-xl backdrop-blur-md">
                  <div className="space-y-1">
                    ...
                    <p className="text-[7px] text-gray-400 font-bold tracking-widest">{g.table} — <span className={g.status === 'CONFIRMADO' ? 'text-purple-400 font-black' : 'text-amber-400'}>{g.status}</span></p>
                  </div>
        ...
        {activeTab === 'budget' && (
          <div className="space-y-5">
            <form onSubmit={handleAddBudget} className="bg-[#0e0e10] border border-white/15 p-4 sm:p-6 rounded-[1.75rem] sm:rounded-[2rem] grid grid-cols-1 sm:grid-cols-4 gap-3 font-bold items-center shadow-[0_0_30px_rgba(0,0,0,0.3)]">
              <input placeholder="CONCEPTO (EJ: CATERING)" value={newBudget.concept} onChange={e => setNewBudget({...newBudget, concept: e.target.value})} className="bg-white/5 border border-white/15 rounded-xl p-3.5 text-[9px] text-white outline-none focus:border-purple-500 tracking-widest uppercase placeholder:text-gray-600" required />
              <input type="number" placeholder="ESTIMADO ($)" value={newBudget.estimated} onChange={e => setNewBudget({...newBudget, estimated: e.target.value})} className="bg-white/5 border border-white/15 rounded-xl p-3.5 text-[9px] text-white outline-none focus:border-purple-500 tracking-widest uppercase placeholder:text-gray-600" />
              <input type="number" placeholder="REAL ($)" value={newBudget.actual} onChange={e => setNewBudget({...newBudget, actual: e.target.value})} className="bg-white/5 border border-white/15 rounded-xl p-3.5 text-[9px] text-white outline-none focus:border-purple-500 tracking-widest uppercase placeholder:text-gray-600" />
              <button type="submit" className="py-3.5 bg-purple-600 text-white rounded-xl text-[8px] font-black tracking-widest hover:bg-purple-500 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg">
            ...
            <div className="bg-[#0e0e10] border border-white/15 rounded-[2rem] p-4 sm:p-6 overflow-x-auto shadow-[0_0_40px_rgba(0,0,0,0.4)] space-y-4 backdrop-blur-md">
              <table className="w-full text-left border-collapse min-w-[320px]">
                <thead>
                  <tr className="border-b border-white/10 text-[7px] text-purple-400 tracking-[0.3em] font-bold">
                      ...
                      return (
                        <tr key={b.id}>
                          <td className="py-3.5 text-white">
                            {isEditing ? (
                              <input value={editBudgetValues.concept} onChange={e => setEditBudgetValues({...editBudgetValues, concept: e.target.value})} className="bg-white/5 border border-purple-500/50 rounded-lg p-2 text-white outline-none w-full" />
                            ) : b.concept}
                          </td>
                          <td className="py-3.5 text-gray-300">
                            {isEditing ? (
                              <input type="number" value={editBudgetValues.estimated} onChange={e => setEditBudgetValues({...editBudgetValues, estimated: e.target.value})} className="bg-white/5 border border-purple-500/50 rounded-lg p-2 text-white outline-none w-20" />
                            ) : `$${b.estimated}`}
                          </td>
                          <td className="py-3.5 text-white">
                            {isEditing ? (
                              <input type="number" value={editBudgetValues.actual} onChange={e => setEditBudgetValues({...editBudgetValues, actual: e.target.value})} className="bg-white/5 border border-purple-500/50 rounded-lg p-2 text-white outline-none w-20" />
                            ) : `$${b.actual}`}
                          </td>
              ...
              {budgetItems.length > 0 && (
                <div className="pt-3 border-t border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center text-[8px] font-black tracking-widest px-1 gap-2">
                  <span className="text-purple-400">TOTALES:</span>
                  <div className="flex gap-4">
                    <span className="text-gray-400">EST: <span className="text-white">${totalEstimated}</span></span>
                    <span className="text-gray-400">REAL: <span className="text-purple-300">${totalActual}</span></span>
                  </div>
                </div>
              )}
            </div>
        ...
        {showPhotoModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/85 backdrop-blur-xl p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className="bg-[#0e0e10] w-full max-w-md p-6 sm:p-8 rounded-[2.5rem] border border-white/15 relative shadow-[0_0_50px_rgba(168,85,247,0.3)] space-y-5 max-h-[90vh] overflow-y-auto"
            >
              ...
              <div className="space-y-1 text-center">
                <span className="text-[7px] tracking-[0.3em] font-bold text-purple-400 uppercase">CONFIGURACIÓN</span>
              ...
              <form onSubmit={handleUpdateCover} className="space-y-4 font-bold">
                <div className="space-y-1.5">
                  ...
                  <label className="w-full bg-white/5 border border-white/15 hover:border-purple-500/50 rounded-xl p-3.5 flex items-center justify-center gap-2 cursor-pointer transition-all text-[9px] text-gray-200">
                ...
                <input placeholder="https://..." value={coverUrl.startsWith('data:') ? '' : coverUrl} onChange={e => setCoverUrl(e.target.value)} className="w-full bg-white/5 border border-white/15 rounded-xl p-3.5 text-[9px] text-white outline-none focus:border-purple-500 tracking-widest uppercase placeholder:text-gray-600" />
                ...
                <button type="submit" className="w-full py-3.5 bg-purple-600 text-white rounded-xl text-[8px] font-black tracking-[0.3em] uppercase hover:bg-purple-500 transition-all shadow-xl cursor-pointer">
                  GUARDAR
                </button>
              </form>
            </motion.div>
