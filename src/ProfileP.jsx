  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        className="relative w-full max-w-sm bg-[#070709] border border-white/10 rounded-[3rem] shadow-2xl overflow-hidden p-8 flex flex-col items-center text-center backdrop-blur-2xl uppercase"
      >
        ...
        <div className="mb-8 text-center">
          <div className="w-24 h-24 rounded-full bg-white/10 p-1 mx-auto mb-4 border border-white/10">
        ...
        <div className="w-full bg-white/[0.03] border border-white/10 rounded-2xl p-4 flex items-center gap-3 mb-8">
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="px-6 md:px-12 pb-8 relative -mt-14 md:-mt-20 flex flex-col md:flex-row items-center md:items-end justify-between gap-6 z-20">
            <div className="flex flex-col md:flex-row items-center md:items-end gap-6 text-center md:text-left">
              <div className="relative w-28 h-28 md:w-36 md:h-36 rounded-full border-4 border-[#070709] overflow-hidden bg-black shadow-2xl flex-shrink-0">
              ...
              <div className="space-y-2 pb-1">
                ...
                <p className="text-gray-400 text-[10px] md:text-xs tracking-[0.3em] font-bold">{user.job || 'PROFESIONAL'}</p>
      ...
      <main className="max-w-[1200px] mx-auto px-6 md:px-12 py-12 grid lg:grid-cols-12 gap-10 relative z-10 w-full">
        ...
        <div className="lg:col-span-4 space-y-8">
          {user.completedCourses && user.completedCourses.length > 0 && (
            <div className="bg-[#0c0c0e] border border-white/10 rounded-3xl p-6 space-y-6 shadow-xl">
              <h3 className="text-[10px] text-gray-400 uppercase tracking-[0.4em] font-black border-l-2 border-white pl-4">Certificaciones Classcode®</h3>
              <div className="space-y-3">
                {user.completedCourses.includes('cert_fotografia_triangulo') && (
                  <div className="flex items-center gap-3 bg-white/[0.03] border border-white/10 px-4 py-3 rounded-2xl">
        ...
        <div className="lg:col-span-8 space-y-12">
          <section className="bg-[#0c0c0e] border border-white/10 rounded-3xl p-6 md:p-8 space-y-8 shadow-xl">
            <h3 className="text-[10px] text-gray-400 uppercase tracking-[0.4em] font-black border-l-2 border-white pl-4">Portfolio</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {user.photos?.slice(1, 13).map((url, i) => (
                <div key={i} className="aspect-square rounded-2xl overflow-hidden border border-white/10 bg-black group relative shadow-xl">
      ...
      <AnimatePresence>
        {showChat && (
          <motion.aside initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }}
            className="fixed right-6 bottom-6 w-[360px] max-w-[95vw] h-[550px] bg-[#0c0c0e] border border-white/10 rounded-[3rem] shadow-2xl z-[60] flex flex-col overflow-hidden"
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.senderId === auth.currentUser?.uid ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-4 rounded-[1.5rem] max-w-[85%] text-[11px] leading-relaxed tracking-wide ${m.senderId === auth.currentUser?.uid ? 'bg-white text-black font-bold' : 'bg-white/[0.03] text-gray-300 border border-white/10'}`}>
            ...
            <form onSubmit={sendDirectMessage} className="p-5 bg-black/40 border-t border-white/10 flex gap-3">
              <input value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="ESCRIBIR..." className="flex-1 bg-black border border-white/10 rounded-full py-4 px-6 text-[10px] text-white uppercase outline-none focus:border-white" />
              <button type="submit" className="p-4 bg-white text-black rounded-full hover:bg-gray-200 transition-all"><Send size={16}/></button>
            </form>
          </motion.aside>
        )}
      </AnimatePresence>
        ...
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className="bg-[#0c0c0e] w-full max-w-lg p-10 md:p-14 rounded-[3rem] border border-white/10 relative shadow-2xl"
            >
              ...
              <div className="space-y-4 font-bold">
                <input placeholder="TIPO DE EVENTO" className="w-full bg-white/[0.03] border border-white/10 rounded-2xl p-5 text-[10px] text-white uppercase outline-none focus:border-white tracking-widest" onChange={e => setFormData({...formData, tipoEvento: e.target.value})} />
                <div className="grid grid-cols-2 gap-4">
                  <input placeholder="FECHA / FECHAS" className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 text-[10px] text-white uppercase outline-none focus:border-white" onChange={e => setFormData({...formData, fecha: e.target.value})} />
                  <input placeholder="DURACIÓN ESTIMADA" className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 text-[10px] text-white uppercase outline-none focus:border-white" onChange={e => setFormData({...formData, duracion: e.target.value})} />
                </div>
                <input placeholder="DIRECCIÓN EXACTA DEL EVENTO" className="w-full bg-white/[0.03] border border-white/20 rounded-2xl p-5 text-[10px] text-white uppercase outline-none focus:border-white font-black tracking-widest" onChange={e => setFormData({...formData, direccion: e.target.value})} />
                <textarea placeholder="DETALLES ADICIONALES (EQUIPO, REQUERIMIENTOS...)" className="w-full bg-white/[0.03] border border-white/10 rounded-3xl p-5 text-[10px] text-white uppercase h-32 resize-none outline-none focus:border-white tracking-widest" onChange={e => setFormData({...formData, detalles: e.target.value})} />
              </div>
